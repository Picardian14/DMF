...
sub_experiment_name = "GridWhyNoise";
% Dataset values
...
params.scale = [0 2]; % Here only optimizing gain
params.bias = [-0.9 0];
...
% Actual parameter
weight_space = linspace(params.scale(1), params.scale(2), n_weigth);
bias_space = linspace(params.bias(1), params.bias(2), n_bias);
weight_range = n_weigth-7:n_weigth;
bias_range = 3:n_bias;
...
for cur_weight=weight_range
    thispars=params;       
    %weight_space(cur_weight);
    %fprintf("\n")
    for cur_bias=bias_range   
        %bias_space(cur_bias);
        thispars.scale = weight_space(cur_weight);                                           
        thispars.bias = bias_space(cur_bias);                 
        % Simulating  
        simulations=zeros(N,N,params.NSIM);
        fprintf("%.2f - %.2f | ", thispars.scale, thispars.bias);
...
[fc_error_bias, weight_min] = min(local_fc_error_corr);
...
for w_index=1:length(best_weights)
    thispars.scale = best_weights(w_index);
    thispars.bias = bias_space(w_index);
    parfor nsub=1:thispars.NSUB
        ...
        simulationsFC5(:, :, nsub, w_index) = corrcoef(BOLDNM);
    end
end
figure;
