...
for nsub=indexsub
    ...
    WdataF(:,:,nsub) = zscore(WdataF(:, :, nsub));
...
%%
for cur_weight=1%weight_range    
    
    for cur_bias=1%bias_range   
        %bias_space(cur_bias);
        thispars=params;  
        thispars.G = 1.9;%weight_space(cur_weight);                                           
        thispars.alpha = 0.4;%bias_space(cur_bias);       
